...
function WorkoutLog({ userDetails }) {
 ...
 function calculateCalories(){
  if(workoutType=='Balance & Stability'){
    setCaloriesBurnt(workoutMinutes*3);
  }else if(workoutType=='Flexibility Training'){
    setCaloriesBurnt(workoutMinutes*5);
  }else  if(workoutType=='Strength Training'){
    setCaloriesBurnt(workoutMinutes*4);
  }
  else if(workoutType=='Cardio'){
    setCaloriesBurnt(workoutMinutes*9);
  }
 }
 ...
 useEffect(()=>{
  ...
  //     }
  //     console.log(userId);
  //     const uri = 'http://localhost:8000/api/workoutLog/' + userId;
  ...
  const handleSubmit = useCallback (async (e) => {
    e.preventDefault(); 
    if(workoutDate && workoutType && workoutMinutes &&caloriesBurnt && startTime ){
    // if(workoutDate && workoutType && workoutName && workoutMinutes &&caloriesBurnt && startTime ){
      ...
      try{
        ...
        const response = await fetch('http://localhost:8000/api/workoutLogGet', {
          method: 'POST',
          headers: {
   ...
   }, [workoutDate, workoutType, workoutMinutes, caloriesBurnt, startTime, userName, todayWorkoutData])
