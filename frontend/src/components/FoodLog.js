...
function FoodLog() {
  ...
  const fetchFoodData = async (query) => {
    ...
    axios.get('https://api.api-ninjas.com/v1/nutrition', {
      params: { query },
      headers: { 'X-Api-Key': 'zkCtMzOGRSPI1+xkCMnKJw==6k55zkGumJinfmRE' }
    })
      .then(response => {
  ...
  const handleSubmit = useCallback (async (e) => {
    ...
    if(!foodData[0] || foodName.toLowerCase() !== foodData[0].name.toLowerCase()){
      ...
      console.log(foodObj);
      try{
        const response = await fetch('http://localhost:8000/api/foodLogGet', {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log(userId);
        const uri = 'localhost:8000/api/foodlog/' + userId;
